...
export const COLOR_PARAM_SCHEMAS = {
  oxide: {
    knobs: [
      { key: 'corr',  label: 'Corrosion', min: 0, max: 1, step: 0.01, default: 0.32, tip: 'Corrosion blend. 0 = fresh polished metal. 1 = fully aged patina (dark, mottled).' },
      { key: 'metal', label: 'Metal',     min: 0, max: 1, step: 0.01, default: 0.66, tip: 'Metal type. 0 = copper / verdigris. 0.5 = iron / rust. 1 = silver / tarnish.' },
      { key: 'rough', label: 'Rough',     min: 0, max: 1, step: 0.01, default: 0.15, tip: 'Surface roughness noise. 0 = smooth metal. 1 = pitted, granular surface.' },
      { key: 'sheen', label: 'Sheen',     min: 0, max: 1, step: 0.01, default: 0.6,  tip: 'Edge specular highlight. 0 = matte. 1 = polished metal with bright edges along luma transitions.' },
    ],
    toggles: [],
  ...
  synth: {
    knobs: [
      { key: 'warm', label: 'Warmth',    min: 0, max: 1, step: 0.01, default: 0.2, tip: 'Warm-cool color bias inside each band. Low = cool (blue / teal lean). High = warm (red / orange lean).' },
      ...
      { key: 'res',  label: 'Res',       min: 0, max: 1, step: 0.01, default: 0.9, tip: 'Resonance modulation inside each band. 0 = clean steps. 1 = strong sinusoidal brightness ripples per band.' },
      { key: 'dyn',  label: 'Dyn Range', min: 0, max: 1, step: 0.01, default: 0.36, tip: 'Dynamic range / gamma. Low = compressed midtones (flat, washed). High = stretched midtones (punchy, contrasty).' },
    ],
    toggles: [],
  ...
  newsprint: {
    knobs: [
      ...
      { key: 'threshold', label: 'Threshold', min: 0, max: 1, step: 0.01, default: 0.5, tip: 'Halftone dot threshold. 0 = heavy coverage, dots appear even in lights. 1 = sparse, only deep shadows get dots.' },
    ],
    toggles: [],
    order: ['scale', 'hueA', 'hueB', 'threshold'],
  },
  sketch: {
    knobs: [
      { key: 'ink',    label: 'Ink',    min: 0, max: 1, step: 0.01, default: 0.1,  tip: 'Line boldness. Low = faint pencil whisper. High = heavy inked hatching that bites into the shadows.' },
      { key: 'color',  label: 'Color',  min: 0, max: 1, step: 0.01, default: 1.0,  tip: 'Colored-pencil amount. 0 = graphite black-and-white sketch. 1 = full colored crosshatch from the source.' },
      { key: 'stroke', label: 'Stroke', min: 0, max: 1, step: 0.01, default: 0.0,  tip: 'Hatch stroke length. Low = tight fine scribble. High = long sweeping pen strokes.' },
      { key: 'wobble', label: 'Wobble', min: 0, max: 1, step: 0.01, default: 0.6,  tip: 'Hand-shake. The whole drawing jitters as if sketched live. 0 = steady hand, 1 = caffeinated.' },
    ],
    toggles: [],
  ...
  okdrift: {
    ...
    toggles: [
      { key: 'relType', label: 'Harmony', default: 0, options: [
        ...
        { value: 6, label: 'Split-Comp', tip: 'Split-Complementary — base hue plus two flanks at 150° and 210°. Cinematic tension without the harshness of straight Comp.' },
        { value: 7, label: 'Spectral',   tip: 'Spectral — stops spread evenly across the full colour wheel. Rainbow effect that rides the L grade dark→bright.' },
        { value: 8, label: 'Duotone',    tip: 'Duotone — two tight hue clusters at base and complement. Strong two-tone contrast with subtle warmth variation within each cluster.' },
        { value: 9, label: 'Pentadic',   tip: 'Pentadic — five hue poles at 72° intervals. Dense geometric harmony; works best with 5–10 stops.' },
      ]},
    ],
...
export const FX_PARAM_SCHEMAS = {
  rgbdelay: {
    feedback: true,
    knobs: [
      { key: 'delay',  label: 'Delay',  min: 0, max: 1, step: 0.01, default: 0.5,  tip: 'Master channel persistence. How long each colour is held in the buffer. 0 = no memory. Near 1 = near-infinite trail.' },
      { key: 'spread', label: 'Spread', min: 0, max: 1, step: 0.01, default: 1.0,  tip: 'Channel divergence. R decays fastest (shortest delay), B slowest (longest). 0 = all channels equal. 1 = maximum separation.' },
      { key: 'drift',  label: 'Drift',  min: 0, max: 1, step: 0.01, default: 0.0,  tip: 'Spatial drift. R and B sample the feedback buffer at slowly orbiting offsets — on motion this pulls each colour into its own ghost halo.' },
  ...
  tunnel: {
    feedback: true,
    knobs: [
      { key: 'zoom',   label: 'Zoom',   min: 0, max: 1, step: 0.01, default: 0.83, tip: 'How fast echoes recede into the tunnel. 0 = barely moving. 1 = fast infinite-zoom plunge.' },
      ...
      { key: 'drift',  label: 'Hue',    min: 0, max: 1, step: 0.01, default: 0.07, tip: 'Hue drift per echo generation. The tunnel shifts color as it deepens — the classic psychedelic feedback rainbow.' },
      { key: 'mix',    label: 'Mix',    min: 0, max: 1, step: 0.01, default: 0.1,  tip: 'Echo persistence. 0 = faint single echo. 1 = near-infinite hall of mirrors.' },
    ],
    toggles: [],
  ...
  wobbletape: {
    feedback: true,
    knobs: [
      { key: 'flutter', label: 'Flutter', min: 0, max: 1, step: 0.01, default: 0.34, tip: 'Wow/flutter strength — how hard the tape drags sideways each frame.' },
      { key: 'accum',   label: 'Accum',   min: 0, max: 1, step: 0.01, default: 1.0,  tip: 'How much each frame re-displaces the last. High = the image progressively stretches into taffy before the next snap.' },
      { key: 'snap',    label: 'Snap',    min: 0, max: 1, step: 0.01, default: 0.0,  tip: 'Tracking-pulse rate. Each pulse snaps the smear back to a clean image. 0 = rare snaps (long degradation). 1 = constant snapping.' },
      { key: 'tear',    label: 'Tear',    min: 0, max: 1, step: 0.01, default: 1.0,  tip: 'Chroma tear. R and B channels drag at different rates, fringing the smear like misaligned tape heads.' },
    ],
    toggles: [],
  ...
  drag: {
    ...
    knobs: [
      ...
      { key: 'decay',  label: 'Decay',   min: 0, max: 1, step: 0.01, default: 0.71, tip: 'Trail persistence. Near 1 = long comet tails. Low = trails die in a few frames.' },
  ...
  godrays: {
    knobs: [
      { key: 'cx',      label: 'Ctr X',    min: 0, max: 1, step: 0.01, default: 0.25, tip: 'Light source X position. 0 = left edge. 1 = right edge. 0.5 = center.' },
      { key: 'cy',      label: 'Ctr Y',    min: 0, max: 1, step: 0.01, default: 0.8,  tip: 'Light source Y position. 0 = bottom. 1 = top. Set high for a sun-from-above look.' },
      { key: 'intens',  label: 'Intensity',min: 0, max: 1, step: 0.01, default: 0.25, tip: 'Ray brightness. 0 = subtle. 1 = heavy volumetric light bloom.' },
      { key: 'density', label: 'Density',  min: 0, max: 1, step: 0.01, default: 0.33, tip: 'Ray reach. 0 = short tight shafts near source. 1 = long sweeping rays across the full frame.' },
    ],
    toggles: [],
  ...
  decayflow: {
    knobs: [
      { key: 'speed',  label: 'Speed',  min: 0, max: 1, step: 0.01, default: 1.0,  tip: 'Flow advection speed. How fast pixels drift along the gradient field.' },
      { key: 'persist',label: 'Persist',min: 0, max: 1, step: 0.01, default: 0.25, tip: 'Trail persistence. How much of the previous advected state carries forward.' },
      { key: 'bright', label: 'Bright', min: 0, max: 1, step: 0.01, default: 1.0,  tip: 'Trail brightness boost on gradient edges.' },
      { key: 'blend',  label: 'Blend',  min: 0, max: 1, step: 0.01, default: 0.25, tip: '0 = pure trail. 1 = source mixed with trail.' },
    ],
    toggles: [],
    order: ['speed', 'persist', 'bright', 'blend'],
  },
  ...
  crt: {
    knobs: [
      { key: 'phosphor', label: 'Phosphor', min: 0, max: 1, step: 0.01, default: 0.6,  tip: '0 = clean. 1 = visible RGB phosphor subpixel grid.' },
      { key: 'bloom',    label: 'Bloom',    min: 0, max: 1, step: 0.01, default: 0.7,  tip: '0 = sharp. 1 = bright areas bleed and glow.' },
      { key: 'barrel',   label: 'Barrel',   min: 0, max: 1, step: 0.01, default: 0.61, tip: '0 = flat. 1 = curved CRT screen barrel distortion.' },
      { key: 'scanline', label: 'Scanline', min: 0, max: 1, step: 0.01, default: 0.2,  tip: '0 = no scanlines. 1 = heavy dark horizontal lines.' },
    ],
    toggles: [],
  ...
  crtrolling: {
    knobs: [
      { key: 'freq',   label: 'Freq',   min: 0, max: 1, step: 0.01, default: 0.0,  tip: '0 = single giant wave. 1 = tight rapid rippling.' },
      { key: 'amp',    label: 'Amp',    min: 0, max: 1, step: 0.01, default: 0.32, tip: 'Horizontal displacement amplitude. 0 = none. 1 = heavy distortion.' },
      { key: 'chroma', label: 'Chroma', min: 0, max: 1, step: 0.01, default: 0.5,  tip: 'R/B chromatic separation on the wave crests. 0 = clean. 1 = full color split.' },
      { key: 'speed',  label: 'Speed',  min: 0, max: 1, step: 0.01, default: 0.32, tip: 'Rolling speed over time. 0 = frozen wave. 1 = fast scrolling distortion.' },
    ],
    toggles: [],
  ...
  degrade: {
    knobs: [
      ...
      { key: 'pixelate', label: 'Pixelate',min: 0, max: 1, step: 0.01, default: 0.41, tip: '0 = full resolution. 1 = chunky pixelated macroblocks.' },
    ],
    toggles: [],
  ...
  noise: {
    knobs: [
      { key: 'amount', label: 'Amount', min: 0, max: 1, step: 0.01, default: 0.1,  control: 'slider', tip: 'Grain amount. 0 = clean. 1 = heavy noise.' },
      { key: 'size',   label: 'Size',   min: 0, max: 1, step: 0.01, default: 0.05, control: 'slider', tip: 'Grain size. 0 = fine 35mm. 1 = chunky 8mm.' },
  ...
  sharpen: {
    knobs: [
      { key: 'strength', label: 'Strength', min: 0, max: 1, step: 0.01, default: 0.8,  tip: 'Sharpening amount. Higher = more edge detail pulled.' },
      { key: 'radius',   label: 'Radius',   min: 0, max: 1, step: 0.01, default: 0.63, tip: 'Kernel blur radius. Wider radius = larger-scale detail enhanced.' },
      ...
      { key: 'luma',     label: 'Luma',     min: 0, max: 1, step: 0.01, default: 0.13, tip: '0 = sharpen all RGB channels. 1 = sharpen luma only (preserves hue, avoids chroma noise).' },
    ],
    toggles: [],
